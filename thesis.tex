...
Die Abbildung \ref{fig:overUnderFitting} zeigt drei Graphen, die das Problem erläutern. Jeder Graph visualisiert abstrakt, wie unterschiedlich das KI-System anhand der Trainingsdaten die einzelnen Punkte in zwei Cluster unterteilt.
%%% evtl. Graphen erklären, x ist Kategorie 1, o Kategorie 2, links vom Graphen ist Kategorie 1 rechts Kategorie 2
\\\\
Die KI im linken Graphen ist unter angepasst, da das Modell mit den Trainingsdaten zu schlecht abgestimmt wurde. Hier wurden zu wenig Punkte richtig abgedeckt, da zu wenige Muster zwischen den Eigenschaften und dem Label erkannt wurden. Solche KI-Modelle neigen zu hohen Verzerrungen der Ergebnisse und teilt die einzelnen Trainingsdaten nur oberflächlich ein.
\\\\
Der rechte Graph hingegen kategorisiert alle Punkte richtig ein. Unter dieser Annahme könnte man denken, es wäre der beste Graph. Hier spricht man allerdings von einer Überanpassung, da das Modell zu gut auf die Trainings- und Testdaten abgestimmt wurde. In der Praxis liefert dieses Model schlechte Ergebnisse. Verantwortlich dafür ist die Integration der Outliner, die starke Abweichungen der eigenen Kategorie beinhalten. Solche Modelle besitzen eine hohe Varianz der Vorhersagen\cite{overUnderfitting}.%%%Varianz der Vorhersagen = hohe Fehleranfälligkeit
\\\\
Beispielsweise wird eine KI mit Deep Learning darauf trainiert, einen Schrank auf einem Bild zu erkennen. Ist das KI-System über angepasst, beinhaltet es zu viele Eigenschaften der Outliner beispielsweise Sessel ähnliche Schränke. Das KI-Programm erkennt alle Trainings- und Testdaten perfekt, sobald man jedoch anfängt mit realen Daten zu testen, ist das Programm bei überschneidenden Eingaben sehr fehleranfällig. Outliner ähnliche Daten, wie in diesem Beispiel ein normaler Sessel, werden vom Programm als Schränke fehlinterpretiert, da das Modell zuvor über angepasst wurde. \\
\\\\
...
%%% evtl. \cite{sahoo} hier erklären
%In DL-Algorithmen ist solche eine Umsetzung nicht trivial, aber das Beispiel von Sahoo et al. \cite{sahoo} zeigt auch die Machbarkeit auch in diesem Feld.
\\\\
...
\\\\
Eine Möglichkeit um das zu verhindern, bietet die Cross Validierung. 
\begin{figure}[h]
	\begin{center}
...
%%% Datengeneralisierung kucken, ob drinn ist
...
\begin{figure}[h]
	\begin{center}
		\includegraphics[width=14cm]{Bilder/polynomeRegression.png}
		\caption{Polynominale Regression}
		\label{fig:polyReg}
	\end{center}
\end{figure}
Anhand einer polynomialen Regression kann man verdeutlichen, wie Underfitting vermieden werden könnte. Um dies zu erreichen, muss der erwartete Wert einer Variable als Polynom d-ten Grades modelliert werden, welches dann wiederum das allgemeine Polynom ergibt. Wenn wir den Grad ${d}$ des Polynoms erhöhen, nimmt der Trainingsfehler generell ab. Die allgemeine Gleichung eines Polynoms ist folgende:
%%% Quelle?

${\displaystyle y=\beta_{0}+\beta_{1}x+\beta_{2}x^{2}+...+\beta_{n}^n+\epsilon}$

Aber hier muss aufgepasst werden. Denn, wie in der Abbildung \ref{fig:polyReg} zu sehen, sinkt zur gleichen Zeit der Cross Validierungsfehler, solange wir ${d}$ bis zu einem gewissen Punkt erhöhen, nach diesem Punkt steigt der Fehler und Overfitting wird das Problem.
\subsection{Overfitting}
%%% Evtl Problem: Ähnlichkeit der Daten aufgreifen -> Outliner sind egal
Eine Möglichkeit um Overfitting zu vermeiden, bietet die bereits in Abschnitt \ref{section:validate} Cross Validierung. %%% Warum löst das das Problem
Eine andere kann durch das Erhöhen der Flexibilität des Models erreicht werden. Um die Flexibilität zu erhöhen, können Regulierungstechniken benutzt werden.
\\\\
Durch die Regulierung wird eine Strafe für bestimmte Parameter angehängt, um die Komplexität eines Modells zu verhindern. Der Regulierungsparameter (${\lambda}$) bestraft alle Parameter mit Ausnahme des markierten Bereichs in Abbildung \ref{fig:regFunc}(lila Umrahmt), sodass das Modell die Daten verallgemeinert und nicht überanpasst. 
%%% Erklären, warum nicht den makierten Bereich bestrafen wird %%% Loss-Funktion nenne verlinken, 
...
\begin{figure}[h]
	...
	\end{center}
\end{figure}
%%% Strafe wird drei mal erklärt.
Wenn in der Abbildung \ref{fig:regFunc} die Komplexität steigt, wird durch die Regulierung eine Strafe für höhere Terme hinzugefügt. Das hat zur Folge, dass die Wichtigkeit solcher langen Terme sinkt und zugleich die Komplexität des Models weniger wird.
\\\\
